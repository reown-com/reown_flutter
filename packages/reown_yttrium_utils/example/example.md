...
class _MyHomePageState extends State<MyHomePage> {
  final _yttriumUtils = ReownYttriumUtils();
  ...
  Future<void> _initializeYttrium() async {
    try {
      final success = await _yttriumUtils.init(
  ...
  Future<void> _checkTokenBalance() async {
    try {
      final balance = await _yttriumUtils.erc20TokenBalance(
  ...
  Future<void> _estimateFees() async {
    try {
      final fees = await _yttriumUtils.estimateFees(
  ...
  Future<void> _prepareAndExecuteTransaction() async {
    try {
      ...
      final preparedTx = await _yttriumUtils.prepareDetailed(
        chainId: 'eip155:x',
      ...
      final executionDetails = await _yttriumUtils.execute(
