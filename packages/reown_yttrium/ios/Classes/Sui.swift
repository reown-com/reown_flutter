...
class Sui {
    ...
    static func initialize(_ params: Any, result: @escaping FlutterResult) {
        guard let dict = params as? [String: Any] else {
            result(FlutterError(code: "Sui.initialize", message: "Invalid parameters: not a map", details: params))
            return
        }
        ...
        guard
            ...
            result(FlutterError(code: "Sui.initialize", message: "Missing required fields", details: dict))
            return
        }
